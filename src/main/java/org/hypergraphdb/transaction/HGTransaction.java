...
public final class HGTransaction implements HGStorageTransaction {
	...
	/**
	 * A commit can proceed only if none of the values we've read during the
	 * transaction has changed (i.e. has been committed) since we read them. In
	 * order words, the latest/current body of each VBox is the same as the one
	 * tagged with this transaction's number.
	 */
	protected boolean validateCommit() {
		if (!readonly)
			for (Map.Entry<VBox<?>, VBoxBody<?>> entry : bodiesRead.entrySet()) {
				// Compare versions instead of 'body' objects because we may
				// have multiple
				// re-loads of the same version of some disk data - we allow
				// that in
	...
	/**
	 * Commit all "written boxes" and return a linked list of the newly attached
	 * bodies to them. Those bodies will be garbage collected eventually when it
	 * is determined that no current transaction (or future) could be using them
	 * (see ActiveTransactionRecord.clean()).
	 */
	protected Cons<VBoxBody<?>> doCommit() {
		...
		for (Map.Entry<VBox<?>, Object> entry : boxesWritten.entrySet()) {
			...
			VBoxBody<?> newBody = vbox.commit(this,
					(newValue == NULL_VALUE) ? null : newValue, number);
	...
	private void fatalFailure(Throwable t) {
		...
		// as an OutOfMemory error, so we will just re-throw it. However, in
		// case
		// the caller (the client application) doesn't catch it but attempts
		// some other DB operation (e.g. a graph.close() in a finally block or
		// some
		// such), another transaction could be attempted in an inconsistent
		// state.
		// To prevent that from happening, we disable transaction with the
		// manager:
		context.getManager().setEnabled(false);
		// and since this is not enough (RAM transaction are still available and
		// could
		// go into an infinite loop if the logic behind the transaction numbers
		// is
		// driven into an inconsistent path, we also nullify the transaction
		// record
		// which will throw an NPE on any attempt to start a new transaction.
		context.getManager().mostRecentRecord = null;

		// We'll also just print it out in case it gets swallowed by application
		// code
	...
	HGTransaction(HGTransactionContext context, HGTransaction parent,
			ActiveTransactionsRecord activeTxRecord, HGStorageTransaction impl,
			boolean readonly) {
		this.stran = impl;
	...
	public void commit() throws HGTransactionException {
		...
		// Otherwise this is a top-level transaction, we need to do more serious
		// work.
		...
		if (isWriteTransaction()) {
			...
			try {
				if (validateCommit()) {
					...
					// The commit is already done, so create a new
					// ActiveTransactionsRecord
					ActiveTransactionsRecord newRecord = new ActiveTransactionsRecord(
							number, bodiesCommitted);
					...
					// This assignment is need to decrementRunning in the finish
					// method below.
				...
				throw rethrowme;
			} catch (Throwable t) // this should never happen, we're not
									// expecting any exceptions here
			{
				fatalFailure(t);
			} finally {
