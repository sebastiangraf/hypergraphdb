...
/**
 * <p>
 * A <code>LinkCondition</code> constraints the query result set to links pointing to a target set of atoms.
 * The target set is specified when the condition is constructed through an array of <code>HGHandle</code>s.
 * </p>
 * 
    ...
    public boolean satisfies(HyperGraph hg, HGHandle handle) {
        ...
        if (hg.isLoaded(handle)) {
            ...
            if (!(atom instanceof HGLink)) {
                return false;
            } else if (atom instanceof HGBergeLink) {
                // Hack to check children of BergeLinks only, this is necessary for the Versacloud
                // implementation
                HGBergeLink link = (HGBergeLink)atom;
                for (int i = 0; i < link.getHead().size(); i++)
